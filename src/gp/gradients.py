...
def posterior_gradient(gp: GaussianProcess, x_star: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        x_star: (d,) input point at which to evaluate the gradient.
    ...
    # predict() expects shape (m, d) — unsqueeze adds the batch dimension.
    # x.unsqueeze(0) turns (d,) into (1, d).
    mu, _ = gp.predict(x.unsqueeze(0))

    # mu has shape (1,). autograd.grad requires a scalar output,
    # so we index the single element with mu[0].
    # create_graph=False: we only need first-order gradients here.
